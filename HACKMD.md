...
5. 再問 username，為你的AI bot建立一個系統代號, 實際上在對話窗看到的AI bot會是第四步, Telegram強制要求取一個以 `bot` 結尾的名稱（例如 `my_ai_helper_bot`）

6. 它會回覆一串 token，格式像 `7123456789:AAH-xxxxx...`
7. 複製這串 token，存在起來. 在手機端, 我是用email的方式將token寄出來, 然後在Windows 11收信, 收集到這一長串的token。
...
- [Google Drive 下載](https://drive.google.com/file/d/1K-4B2g-i05vYTf2iW0ptq2XYGnrzvrJ8/view?usp=sharing)
- [GitHub 頁面](https://github.com/liamsysmind/one-click-openclaw)
...
如果被「智慧型應用程式控制」擋住，請到系統設定中關閉「智慧型應用程式控制」後再試一次。
...
完成 ChatGPT 連接後，畫面會跑出一連串英文訊息（Skipping channel setup、Skipping search setup 等等），這是正常的。如果畫面停在類似以下內容很久不動：
...
停止後 bot 不會再回應，也不會被自動重啟。下次要用時再雙擊 START 就好。
...
下載安裝包：[Google Drive](https://drive.google.com/file/d/1K-4B2g-i05vYTf2iW0ptq2XYGnrzvrJ8/view?usp=sharing) ｜ [GitHub](https://github.com/liamsysmind/one-click-openclaw)